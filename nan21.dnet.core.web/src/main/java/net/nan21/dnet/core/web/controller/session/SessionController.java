...
public class SessionController {
	...
	throws Exception{
		try {
			IChangePasswordService service = this.webappContext.getBean(IChangePasswordService.class);
			...
			User user = ((SessionUser)ctx.getAuthentication().getPrincipal()).getUser();
			service.changePassword(user.getUsername(), newPassword, oldPassword, user.getClientId(), user.getClientCode());
			return "{success: true}";
		} catch (Exception e) {
			return this.handleException(e, response);
		}		
