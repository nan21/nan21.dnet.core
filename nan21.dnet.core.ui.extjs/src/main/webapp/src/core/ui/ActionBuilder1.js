...
dnet.base.ActionBuilder.prototype =  {
	...
	,addLoad: function(config) {
		var cfg = config||{};
		var name = cfg.name || "load";
		Ext.applyIf(cfg, { id:Ext.id(), _name_:name, name:name
			,scope:	this.frame
			,dc: this.dc
			,tlb: this.name
		});
		...
		var a = this.frame._getDc_(cfg.dc).actions.query;
		//a.
		this.frame._tlbitms_.add(this.name+"__"+cfg.name, a);  //new Ext.Action(cfg)
		
		this.frame._getDc_(cfg.dc).on("inContextOfEditRecord" , function() {
		...
		this.frame._getDc_(cfg.dc).on("afterSelectedRecordsChanged" , function(dc) {
			if (dc.selectedRecords.length > 0) {
                if(this._tlbitms_.get(cfg.tlb+"__edit_sr")) this._tlbitms_.get(cfg.tlb+"__edit_sr").enable();
                if(this._tlbitms_.get(cfg.tlb+"__copy_sr")) this._tlbitms_.get(cfg.tlb+"__copy_sr").enable();
                if(this._tlbitms_.get(cfg.tlb+"__delete_selected_sr")) this._tlbitms_.get(cfg.tlb+"__delete_selected_sr").enable();
			} else {
               if(this._tlbitms_.get(cfg.tlb+"__edit_sr")) this._tlbitms_.get(cfg.tlb+"__edit_sr").disable();
               if(this._tlbitms_.get(cfg.tlb+"__copy_sr")) this._tlbitms_.get(cfg.tlb+"__copy_sr").disable();
               if(this._tlbitms_.get(cfg.tlb+"__delete_selected_sr")) this._tlbitms_.get(cfg.tlb+"__delete_selected_sr").disable();
			}
	...
	,addNewSr: function(config) {
		var cfg = config||{};
		var name = cfg.name || "new_sr";
		...
		var fn = function() {
			...
			cfg.handler = fn;	
			var a = this.frame._getDc_(cfg.dc).actions.create;
			a.setHandler(fn, this.frame);
			this.frame._tlbitms_.add(this.name+"__"+cfg.name, a); 	
		//this.frame._tlbitms_.add(this.name+"__"+cfg.name, new Ext.Action(cfg)); 
		...
		this.frame._getDc_(cfg.dc).on("inContextOfNewRecord" , function() {
			 ...
			 t.eachKey( function(k) { try {this._tlbitms_.get(k).disable();} catch(e) {} } ,this.frame);
		  } , this.frame );
		return this;	
	}
