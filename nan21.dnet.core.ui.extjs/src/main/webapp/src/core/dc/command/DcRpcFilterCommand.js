...
dnet.base.DcRpcFilterCommand = Ext.extend(dnet.base.AbstractDcAsyncCommand, {
	/**
	 * Call a service on the data-source filter. 
	 * @param serviceName: The name of the data-source service to be executed.
	 * @param options: Specifications regarding the execution of this task.
	 ...
	 */
	 onExecute: function(options) {	
		...
		Ext.Ajax.request({
			url: Dnet.dsAPI(dc.dsName, "json").service, method:"POST", params: p
	...
	,onAjaxSuccess: function(response, options) {
		...
		if (!(s.callbacks && s.callbacks.silentSuccess === true)) {
			dc.fireEvent("afterDoServiceFilterSuccess", dc, response, name, options);
		}					
	}
	...
	,onAjaxFailure: function(response, options) {
		...
		if (!(specs.callbacks && specs.callbacks.silentFailure === true)) {
			dc.fireEvent("afterDoServiceFilterFailure", dc, response, name, options);
		}		
	}
