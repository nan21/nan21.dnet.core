...
public class QueryBuilderWithJpqlTest {
	...
	public void testQueryBuilderWithModelOnly() throws Exception {
		...
		String expectedEql = "BASE-EQL join fetch e.parent1 left join fetch e.parent2 where  e.name not null  and e.sequenceNo > :sequenceNoAlias and e.code like :code and e.code = :code_1 and e.code <> :code_2 and e.code like :code_3 and e.code not like :code_4 and e.id > :id_5 and e.id >= :id_6 and e.id < :id_7 and e.id <= :id_8 and e.id between :id_9_1 and :id_9_2 order by e.code,e.name desc";
