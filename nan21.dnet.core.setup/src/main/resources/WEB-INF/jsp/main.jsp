	...
	<table align=center style='vertical-align: middle; align: center'>
		...
		<tr>
			<td colspan=2 style='padding: 10px; width: 800px;'>
				...
				<table width='100%'>
					<tr>
						<td class="title"><b><u>Current step:</u></b>
							${current_title}</td>
						<td class="text"><b><u>Target:</u></b><br>
							${current_bundle}</td>
					</tr>
					<tr>
						<td class="text" colspan=2><br> <b><u>Description:</u></b>
							${current_description}</td>
					</tr>
					<tr>
						...
						<td
							...
							<form name="frm" action="doSetup" method="post">
								<input type="hidden" name="taskId" value="${taskId}"> <input
									type="hidden" name="bundleId" value="${bundleId}">
								<table cellspacing=5 cellpadding=0 width="100%">
									<c:forEach items="${currentTask.params}" var="item">
										<tr>
											<td class='label'><c:out value='${item.title }' /> : <br>${item.description}</td>

											<td width=150 style="vertical-align: bottom;"><c:if
													test="${item.fieldType=='textfield'}">
													<input name='${paramPrefix}${item.name}' class='field'
														value="${item.value}">
												</c:if> <c:if test="${item.fieldType=='checkbox'}">
									...
									<tr>
										<td colspan=3><input name="save" class='button'
											type="button" onclick="javascript:doSave();" value="Continue"></td>
									</tr>
								</table>
							</form>
						</td>
		...
		<tr>
			<td
				...
				colspan=2><br> Copyright &copy; 2010-2012 Nan21
				Electronics SRL. All Rights Reserved.</td>
		</tr>
	</table>
</body>
<script>
