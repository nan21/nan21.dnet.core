 ...
 * 
 * <p>
 * To send an email in HTML, one should create a <code>HtmlEmail</code>, then
 * use the {@link #setFrom(String)}, {@link #addTo(String)} etc. methods. The
 * HTML content can be set with the {@link #setHtmlMsg(String)} method. The
 ...
 * multipart becomes whichever is supplied rather than a
 ...
 * 
 * <p>
 * Embedded entities are tracked by their name, which for <code>File</code>s is
 * the filename itself and for <code>URL</code>s is the canonical path. It is an
 * error to bind the same name to more than one entity, and this class will
 ...
 */
public class HtmlEmail extends MultiPartEmail implements IHtmlEmail {
	 ...
	 * 
	 * <p>
	 * This method overrides {@link MultiPartEmail#setMsg(String)} in order to
	 * send an HTML message instead of a plain text message in the mail body.
	 * The message is formatted in HTML for the HTML part of the message; it is
	 ...
	 * @throws EmailException
	 *             if msg is null or empty; see javax.mail.internet.MimeBodyPart
	 *             for definitions
	 * @since 1.0
	 */
	public IEmail setMsg(String msg) throws EmailException {
		...
		StringBuffer htmlMsgBuf = new StringBuffer(msg.length()
				+ HTML_MESSAGE_START.length() + HTML_MESSAGE_END.length());

		htmlMsgBuf.append(HTML_MESSAGE_START).append(msg).append(
				HTML_MESSAGE_END);
	 ...
	 * @throws EmailException
	 *             when URL supplied is invalid or if <code> is null
	 ...
	 * 
	 * <p>
	 * This method embeds a file located by an URL into the mail body. It
	 ...
	 * Content-ID returned by the embed function. It is an error to bind the
	 * same name to more than one URL; if the same URL is embedded multiple
	 * times, the same Content-ID is guaranteed to be returned.
	 * 
	 * <p>
	 * While functionally the same as passing <code>URLDataSource</code> to
	 * {@link #embed(DataSource, String, String)}, this method attempts to
	 * validate the URL before embedding it in the message and will throw
	 ...
	 * <code>EmailException</code>. However, it is not guaranteed to catch all
	 * cases, especially when the URL refers to a remote HTTP host that may be
	 * part of a virtual host cluster.
	 * 
	 * @param url
	 *            The URL of the file.
	 * @param name
	 ...
	 * 
	 * @param file
	 *            The <code>File</code> object to embed
	 * @return A String with the Content-ID of the file.
	 * @throws EmailException
	 *             when the supplied <code>File</code> cannot be used; also see
	 *             {@link javax.mail.internet.MimeBodyPart} for definitions
	 * 
	 ...
	 * allows, for instance, to add inline images to the email. Inline files may
	 * be referenced with a <code>cid:xxxxxx</code> URL, where xxxxxx is the
	 * Content-ID returned by the embed function. Files are bound to their
	 * names, which is the value returned by {@link java.io.File#getName()}. If
	 * the same file is embedded multiple times, the same CID is guaranteed to
	 * be returned.
	 * 
	 * <p>
	 * While functionally the same as passing <code>FileDataSource</code> to
	 * {@link #embed(DataSource, String, String)}, this method attempts to
	 * validate the file before embedding it in the message and will throw
	 * <code>EmailException</code> if the validation fails. In this case, the
	 * <code>HtmlEmail</code> object will not be changed.
	 * 
	 * @param file
	 *            The <code>File</code> to embed
	 * @param cid
	 *            the Content-ID to use for the embedded <code>File</code>
	 * @return A String with the Content-ID of the file.
	 * @throws EmailException
	 *             when the supplied <code>File</code> cannot be used or if the
	 *             file has already been embedded; also see
	 *             {@link javax.mail.internet.MimeBodyPart} for definitions
	 * @since 1.1
	 */
	public String embed(File file, String cid) throws EmailException {
		...
		if (inlineEmbeds.containsKey(file.getName())) {
			...
			try {
				...
				throw new EmailException(
						"couldn't get canonical path for file "
		...
		if (!file.isFile()) {
			throw new EmailException("file " + filePath
					+ " isn't a normal file");
		}
		if (!file.canRead()) {
	...
	/**
	 * Embeds the specified <code>DataSource</code> in the HTML using a randomly
	 * generated Content-ID. Returns the generated Content-ID string.
	 * 
	 * @param dataSource
	 *            the <code>DataSource</code> to embed
	 * @param name
	 *            the name that will be set in the filename header field
	 * @return the generated Content-ID for this <code>DataSource</code>
	 * @throws EmailException
	 *             if the embedding fails or if <code>name</code> is null or
	 *             empty
	 ...
	 *            the name that will be set in the filename header field
	 * @param cid
	 *            the Content-ID to use for this <code>DataSource</code>
	...
	private static class InlineImage {
		 ...
		 * and <code>MimeBodyPart</code>.
		 * 
		 * @param cid
		 *            the generated content ID
		 * @param dataSource
		 *            the <code>DataSource</code> that represents the content
		 * @param mbp
		 *            the <code>MimeBodyPart</code> that contains the encoded
		 *            data
		 */
		...
		/**
		 * Returns the <code>MimeBodyPart</code> that contains the encoded
		 * InlineImage data.
		 * 
		 ...
		 * {@inheritDoc}
		 * 
		 * @return true if the other object is also an InlineImage with the same
		 *         cid.
		 */
