...
public abstract class Email implements IEmail {
	 ...
	 * 
	 * @param withTLS
	 *            true if TLS needed, false otherwise
	 * @since 1.1
	 */
	 ...
	 * 
	 * @param jndiName
	 *            name of JNDI ressource (javax.mail.Session type), ressource if
	 *            searched in java:comp/env if name dont start with "java:"
	 * @throws IllegalArgumentException
	 *             JNDI name null or empty
	 * @throws NamingException
	 *             ressource can be retrieved from JNDI directory
	 * @since 1.1
	 */
	...
	public Session getMailSession() throws EmailException {
		if (this.session == null) {
			...
			// MY:START Use the provided mails server properties. If null get them from system 
			Properties properties = null;
			if (this.mailServerProperties == null ) {
				properties =  new Properties(System.getProperties());
			} else {
				properties = (Properties)this.mailServerProperties.clone();
			}			 
			// MY:END
			...
			if (this.socketTimeout > 0) {
				properties.setProperty(MAIL_SMTP_TIMEOUT, Integer
						.toString(this.socketTimeout));
			}

			if (this.socketConnectionTimeout > 0) {
				properties.setProperty(MAIL_SMTP_CONNECTIONTIMEOUT, Integer
						.toString(this.socketConnectionTimeout));
			}
	 ...
	 * @since 1.0
	 */
	public IEmail setTo(Collection aCollection) throws EmailException {
	 ...
	 * @since 1.0
	 */
	public IEmail setCc(Collection aCollection) throws EmailException {
		if (aCollection == null || aCollection.isEmpty()) {
			throw new EmailException("Address List provided was invalid");
		}
	 ...
	 * @since 1.0
	 */
	public IEmail setBcc(Collection aCollection) throws EmailException {
		if (aCollection == null || aCollection.isEmpty()) {
			throw new EmailException("Address List provided was invalid");
		}
	 ...
	 * @since 1.1
	 */
	public IEmail setReplyTo(Collection aCollection) throws EmailException {
		if (aCollection == null || aCollection.isEmpty()) {
			throw new EmailException("Address List provided was invalid");
		}
	 ...
	 * @since 1.0
	 */
	public IEmail setBounceAddress(String email) {
	 ...
	 * @since 1.0
	 */
	public abstract IEmail setMsg(String msg) throws EmailException;
	...
	public void setMailServerProperties(Properties mailServerProperties) {
		...
		this.hostName = this.mailServerProperties.getProperty(MAIL_HOST);
		this.smtpPort = this.mailServerProperties.getProperty(MAIL_PORT); 
		 
		//this.hostName = this.mailServerProperties.getProperty(MAIL_SMTP_USER); 
		//this.hostName = this.mailServerProperties.getProperty(MAIL_SMTP_PASSWORD);
		//this.hostName = this.mailServerProperties.getProperty(MAIL_SMTP_AUTH);
		
		String user = this.mailServerProperties.getProperty(MAIL_SMTP_USER);
	    String password = this.mailServerProperties.getProperty(MAIL_SMTP_PASSWORD);
	    if (user != null && password != null) {
	      this.setAuthentication(user, password);
	    }
	    if(this.mailServerProperties.getProperty(MAIL_TRANSPORT_TLS) != null && 
	    		this.mailServerProperties.getProperty(MAIL_TRANSPORT_TLS).equals("true")) {
	    	this.setTLS(true);
	    }
	    
		try {
			this.setFrom( this.mailServerProperties.getProperty(MAIL_SMTP_FROM));
